...
//		LexFileReader lexFileReader = new LexFileReader("lex.l");
//		Infix2postfixTransformer i2pt = new Infix2postfixTransformer(lexFileReader);
//	
//		System.out.println(i2pt.postfix("."));
	//	System.out.println(i2pt.proBrakets("a-z \t\n0-9"));
		
	}
	
	public Infix2postfixTransformer(LexFileReader lf) {
		lexFileReader = lf ;
	}
		...
		try {
			for ( int i=0; i<regExpr.length(); i++) {
				...
				if ( c == '\\' ) {
					...
					postFixExpr += RegExpr.STAR;
				} else if ( c=='+' ) {
					postFixExpr += RegExpr.ONE_OR_MORE;
				} else if ( c=='?' ) {
					postFixExpr += RegExpr.ONE_OR_NONE;
				} else if ( c==')' ) {
					while ( s.peek()!= '(' ) {
	...
	public char innerCode( char c ) {
		if ( c=='*' )
			return RegExpr.STAR;
		if ( c=='+' )
			return RegExpr.ONE_OR_MORE;
		if ( c=='?')
			return RegExpr.ONE_OR_NONE;
		if ( c=='.')
			return RegExpr.AND;
		if ( c=='|')
			return RegExpr.OR;
