...
public class NFA {
	...
	/**
	 * Construct a NFA with a blank transition table
	 ...
	 * @param regExpr
	 */
	public NFA( String regExpr , LexFileReader lfr) {
		...
		try {
			for ( int i=0; i<postReg.length(); i++ ) {
				...
				if ( symble == RegExpr.AND ) {
					n = (NFA)s.pop();
					NFA m = (NFA)s.pop();
					s.push( and(m, n) );
				} else if ( symble == RegExpr.OR ) {
					n = (NFA)s.pop();
					NFA m = (NFA)s.pop();
					s.push(or(m,n));
				} else if ( symble == RegExpr.STAR ) {
					n = (NFA)s.pop();
					s.push(star(n));
				} else if ( symble == RegExpr.ONE_OR_MORE ){
					n = (NFA)s.pop();
					s.push(oneMore(n));
				} else if ( symble == RegExpr.ONE_OR_NONE ){
					n = (NFA)s.pop();
					s.push(oneNone(n));
				} else {
					n = new NFA(2);
	...
	protected static final char EPSILON = RegExpr.EPSILON;
}
